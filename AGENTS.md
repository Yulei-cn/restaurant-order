...
- Invoice saving and A4 printing are separate: saving never opens a print dialog. The saved invoice enables the optional print button. `FACTURE.css` owns an A4 portrait print layout with 12mm margins, a 180mm centered invoice area, and no controls/login screen; do not apply the 80mm receipt print rules used by the cashier page. Cashier payment recording also never opens a print dialog: after payment the screen offers optional ticket printing and a clear next-customer action.
...
- On the iPad cashier, the formula block is a deliberate two-row layout: the three formulas occupy row one; row two contains four smaller quick-add tiles in this order: pork roujiamo, beef roujiamo, noodle supplement (+1 EUR), and drink supplement (+1 EUR). Do not move these four choices back into later menu categories.
- `index.html` must also work for phone-based table-side staff ordering: keep the cashier total/confirm bar sticky and prominent, use two columns for formula and quick-add tiles on narrow screens, and maintain at least 42px tap targets for quantity controls. Do not shrink phone quantity buttons below a practical touch size.
- The active cashier day workflow is intentionally short: on `index.html`, the top-left toolbar contains `Ouvrir la caisse` / `开班` and `Cloturer et imprimer Z` / `日结 / 打印 Z`, while French/Chinese remain on the right. Opening records the day and leaves the cashier on the ordering page. Closing records the day, then redirects to `cash-register.html`, which is a read-only daily summary and Z-print page with a return-to-cashier link. A cashier day lasts 24 hours from its first opening, not until calendar midnight. Within that window, a mistaken close changes the opening button to `Reouvrir la caisse` / `重新开班`; reopening is allowed and creates an audit record. After 24 hours, the next click on `开班` creates a new cashier day.
