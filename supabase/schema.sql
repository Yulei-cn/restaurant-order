...
begin
  ...
  set status = 'closed', expected_cash = opening_cash + calculated_cash,
      ...
      closing_hash = encode(digest(concat_ws('|', active_day.business_date::text, calculated_cash::text, target_counted::text, target_actor), 'sha256'), 'hex')
  ...
  values ('daily_close', 'cash_register_day', target_actor, jsonb_build_object('business_date', active_day.business_date, 'expected_cash', expected_cash, 'counted_cash', counted_cash, 'variance_cash', variance_cash));
...
begin
  ...
  if found and recent_day.status = 'open' then
    ...
    insert into public.cashier_audit_logs (event_type, entity_type, actor, details)
    values ('daily_reopen', 'cash_register_day', target_actor, jsonb_build_object('business_date', recent_day.business_date));
    ...
    where status = 'open' and opened_at <= timezone('utc', now()) - interval '24 hours';
    ...
    insert into public.cashier_audit_logs (event_type, entity_type, actor, details)
    values ('daily_open', 'cash_register_day', target_actor, jsonb_build_object('business_date', current_date, 'opening_cash', target_opening));
